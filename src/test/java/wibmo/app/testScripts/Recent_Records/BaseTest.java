...
public class BaseTest extends BaseTest1 {
	...
	{
		int rc=ExcelLibrary.getExcelRowCount("./excel_lib/TestData.xls", "Recent_Records");
		for (int i = 0; i <= rc; i++) 
		{
			if (!ExcelLibrary.getExcelData("./excel_lib/TestData.xls", "Recent_Records",i,0).equals(" ")) 
			{
				testData.put(ExcelLibrary.getExcelData("./excel_lib/TestData.xls", "Recent_Records",i,0), ExcelLibrary.getExcelData("./excel_lib/TestData.xls","Recent_Records",i,2));
				testScenario.put(ExcelLibrary.getExcelData("./excel_lib/TestData.xls", "Recent_Records",i,0), ExcelLibrary.getExcelData("./excel_lib/TestData.xls", "Recent_Records",i,1));
			}
		}
	}
	...
	/**
	 * Navigates to Recent Records/Quick Repeat page by previously navigating to Recharge Page. 
	 *
	 * @param data the data
	 */
	public void gotoRecentRecords(String data)
	{
		gotoRecharge(data);
		RechargePage rp =new RechargePage(driver);
		rp.gotoRecentRecords();		
	}
